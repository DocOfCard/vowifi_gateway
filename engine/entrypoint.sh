...
export USIM_READER_MODE USIM_READER_PIN_INDEX USIM_READER_SWU_INDEX USIM_READER_IMS_INDEX
...
export SWU_PROXY_MODE SWU_SOCKS5_HOST SWU_SOCKS5_PORT SWU_SOCKS5_USERNAME SWU_SOCKS5_PASSWORD SWU_SOCKS5_RDNS
...
if [ "${USIM_READER_MODE:-single}" = "modem_bridge" ]; then
  export USIM_READER="${USIM_READER_PIN_INDEX:-0}"
  export USIM_READER_PORT=""
fi
log "starting pin_keeper (reader=$USIM_READER mode=${USIM_READER_MODE:-single})..."
...
SWU_READER_INDEX="${USIM_READER_SWU_INDEX:-${USIM_READER_INDEX:-0}}"
log "starting SWu IKEv2 tunnel (epdg=$SWU_EPDG apn=$SWU_APN reader=$SWU_READER_INDEX proxy=${SWU_PROXY_MODE:-direct})..."
