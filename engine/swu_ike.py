...
import ipaddress
try:
    import socks  # PySocks (python3-pysocks is already installed by engine/Dockerfile)
except ImportError:
    socks = None
...
class swu():
    ...
    def __init__(self, source_address,epdg_address,apn,modem,default_gateway,mcc,mnc,imsi,ki,op,opc,netns,sqn):
        ...
        # VoCat-style SOCKS5 UDP egress for IKE/NAT-T. This is real RFC1928 UDP ASSOCIATE,
        # not HTTP_PROXY. Direct mode preserves the original socket behaviour.
        self.proxy_mode = os.environ.get("SWU_PROXY_MODE", "direct").strip().lower()
        self.socks5_enabled = self.proxy_mode == "socks5"
        self.socks5_host = os.environ.get("SWU_SOCKS5_HOST", "").strip()
        try:
            self.socks5_port = int(os.environ.get("SWU_SOCKS5_PORT", "1080") or 1080)
        except ValueError:
            self.socks5_port = 1080
        self.socks5_username = os.environ.get("SWU_SOCKS5_USERNAME", "") or None
        self.socks5_password = os.environ.get("SWU_SOCKS5_PASSWORD", "") or None
        self.socks5_rdns = os.environ.get("SWU_SOCKS5_RDNS", "0").strip().lower() in ("1","true","yes","on")
        if self.socks5_enabled:
            if socks is None:
                raise RuntimeError("SOCKS5 requested but PySocks is unavailable")
            if not self.socks5_host:
                raise RuntimeError("SOCKS5 requested but SWU_SOCKS5_HOST is empty")
            swu_log("SOCKS5 UDP proxy enabled: %s:%d rdns=%s" %
                    (self.socks5_host, self.socks5_port, self.socks5_rdns))
    ...
    def _new_ike_udp_socket(self, client_address, label):
        """Create direct UDP or an RFC1928 SOCKS5 UDP-ASSOCIATE socket."""
        if not self.socks5_enabled:
            s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            s.bind(client_address)
            s.settimeout(self.timeout)
            self._enable_outer_pmtud(s)
            return s
        s = socks.socksocket(socket.AF_INET, socket.SOCK_DGRAM)
        s.set_proxy(socks.SOCKS5, self.socks5_host, self.socks5_port,
                    rdns=self.socks5_rdns, username=self.socks5_username,
                    password=self.socks5_password)
        # PySocks bind() establishes the TCP control connection and UDP ASSOCIATE.
        # Binding to the configured local IKE port is best-effort; fall back to ephemeral
        # if the proxy implementation/kernel refuses a privileged port.
        try:
            s.bind(client_address)
        except OSError as e:
            swu_log("SOCKS5 %s local bind %r failed (%r); retrying ephemeral" % (label, client_address, e))
            s.bind((client_address[0], 0))
        s.settimeout(self.timeout)
        swu_log("SOCKS5 UDP socket ready for %s via %s:%d" %
                (label, self.socks5_host, self.socks5_port))
        return s

    def create_socket(self,client_address):
        if self.socket_type != UDP:
            exit()
        self.socket = self._new_ike_udp_socket(client_address, "IKE/500")

    def create_socket_nat(self,client_address):
        if self.socket_type != UDP:
            exit()
        self.socket_nat = self._new_ike_udp_socket(client_address, "NAT-T/4500")

    def create_socket_esp(self,client_address):
        if self.socks5_enabled:
            # SOCKS5 cannot carry raw IP protocol 50. Proxy mode always forces RFC3948 NAT-T.
            self.socket_esp = None
            return
...
#### STATES ####
    ...
    def state_1(self, retry = False, cookie = False): #Send IKE_SA_INIT and process answer
        ...
        if self.ike_decoded_header['exchange_type'] == IKE_SA_INIT:
            ...
            if self.socks5_enabled and self.userplane_mode != NAT_TRAVERSAL:
                self.userplane_mode = NAT_TRAVERSAL
                swu_log("SOCKS5 forces RFC3948 NAT-T: IKE/ESP continue on UDP/4500")
