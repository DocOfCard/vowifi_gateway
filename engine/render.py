...
def build_context(cfg):
    ...
    ctx = {
        ...
        # Role-specific PC/SC reader binding. For a normal CCID card all are the same. A modem
        # bridge exposes three virtual readers, each mapped to a separate UICC logical channel.
        "reader_pin": str(cfg.get("reader_pin_index", cfg.get("reader_index", 0))),
        "reader_swu": str(cfg.get("reader_swu_index", cfg.get("reader_index", 0))),
        "reader_ims": str(cfg.get("reader_ims_index", cfg.get("reader_index", 0))),
    }
    return ctx
...
def main():
    ...
    with open(env_path, "w") as f:
        ...
        f.write(f"USIM_READER_MODE={cfg.get('reader_mode','single')}\n")
        f.write(f"USIM_READER_PIN_INDEX={cfg.get('reader_pin_index', cfg.get('reader_index',0))}\n")
        f.write(f"USIM_READER_SWU_INDEX={cfg.get('reader_swu_index', cfg.get('reader_index',0))}\n")
        f.write(f"USIM_READER_IMS_INDEX={cfg.get('reader_ims_index', cfg.get('reader_index',0))}\n")
        ...
        mode = "socks5" if str(proxy.get("mode") or "direct").lower() == "socks5" else "direct"
        f.write(f"SWU_PROXY_MODE={mode}\n")
        f.write(f"SWU_SOCKS5_HOST={str(proxy.get('host') or '').strip()}\n")
        f.write(f"SWU_SOCKS5_PORT={int(proxy.get('port') or 1080)}\n")
        f.write(f"SWU_SOCKS5_USERNAME={str(proxy.get('username') or '')}\n")
        f.write(f"SWU_SOCKS5_PASSWORD={str(proxy.get('password') or '')}\n")
        f.write(f"SWU_SOCKS5_RDNS={1 if proxy.get('rdns', False) else 0}\n")
