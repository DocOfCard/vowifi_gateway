...
def upsert_instance(inst: dict) -> dict:
    ...
    # Optional per-line SOCKS5 ePDG proxy. Blank password on an edit means keep the saved secret.
    if "proxy" in inst:
        old_proxy = existing.get("proxy") or {}
        new_proxy = inst.get("proxy") or {}
        merged_proxy = {**old_proxy, **new_proxy}
        if not str(new_proxy.get("password") or "").strip():
            merged_proxy["password"] = str(old_proxy.get("password") or "")
        mode = str(merged_proxy.get("mode") or "direct").strip().lower()
        merged_proxy["mode"] = "socks5" if mode == "socks5" else "direct"
        merged_proxy["host"] = str(merged_proxy.get("host") or "").strip()
        try:
            merged_proxy["port"] = max(1, min(65535, int(merged_proxy.get("port") or 1080)))
        except (TypeError, ValueError):
            merged_proxy["port"] = 1080
        merged_proxy["username"] = str(merged_proxy.get("username") or "")
        merged_proxy["rdns"] = bool(merged_proxy.get("rdns", False))
        if merged_proxy["mode"] == "socks5" and not merged_proxy["host"]:
            raise ValueError("SOCKS5 proxy server is required")
        inst["proxy"] = merged_proxy

    # Modem bridge compatibility layout. Ubuntu vsmartcard 3.3 exposes two PC/SC slots per
    # VPCD instance, and stacking three independent VPCD instances proved unreliable. The PIN
    # keeper verifies CHV1 once and then stays APDU-idle, so it safely shares SWu's reader; IMS
    # remains on its own UICC logical channel / VPCD instance.
    reader_mode = str(inst.get("reader_mode") or existing.get("reader_mode") or "single").lower()
    inst["reader_mode"] = "modem_bridge" if reader_mode == "modem_bridge" else "single"
    if inst["reader_mode"] == "modem_bridge":
        base_idx = int(inst.get("reader_index", existing.get("reader_index", 0)) or 0)
        old_layout = str(existing.get("reader_bridge_layout") or "")
        requested_pin = int(inst.get("reader_pin_index", existing.get("reader_pin_index", base_idx)) or base_idx)
        requested_swu = int(inst.get("reader_swu_index", existing.get("reader_swu_index", base_idx)) or base_idx)
        requested_ims = int(inst.get("reader_ims_index", existing.get("reader_ims_index", base_idx + 2)) or (base_idx + 2))
        # Auto-migrate the previous experimental 0/1/2 layout once. After the marker exists,
        # explicit operator reader choices are preserved.
        if old_layout != "compat-v2" and (requested_pin, requested_swu, requested_ims) == (base_idx, base_idx + 1, base_idx + 2):
            requested_swu = requested_pin
        inst["reader_pin_index"] = requested_pin
        inst["reader_swu_index"] = requested_swu
        inst["reader_ims_index"] = requested_ims
        inst["reader_bridge_layout"] = "compat-v2"
        inst["reader_port"] = ""
...
def render_instance_json(inst: dict, settings: dict) -> dict:
    ...
    return {
        ...
        "reader_mode": "modem_bridge" if str(inst.get("reader_mode") or "single").lower() == "modem_bridge" else "single",
        "reader_pin_index": int(inst.get("reader_pin_index", inst.get("reader_index", 0)) or 0),
        "reader_swu_index": int(inst.get("reader_swu_index", inst.get("reader_index", 0)) or 0),
        "reader_ims_index": int(inst.get("reader_ims_index", inst.get("reader_index", 0)) or 0),
        ...
        "proxy": {
            "mode": "socks5" if str((inst.get("proxy") or {}).get("mode") or "direct").lower() == "socks5" else "direct",
            "host": str((inst.get("proxy") or {}).get("host") or "").strip(),
            "port": int((inst.get("proxy") or {}).get("port") or 1080),
            "username": str((inst.get("proxy") or {}).get("username") or ""),
            "password": str((inst.get("proxy") or {}).get("password") or ""),
            "rdns": bool((inst.get("proxy") or {}).get("rdns", False)),
        },
    }
