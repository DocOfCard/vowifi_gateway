...
def _modem_pin_override(reader_name: str) -> bool | None:
    """Return authoritative PIN enabled state for the modem-backed virtual reader.

    VERIFY-with-empty-data (63Cx) is only a retry-counter probe and some modem/eUICC
    combinations return it even when PIN1 facility locking is disabled. The modem itself
    exposes the authoritative state via AT+CPIN? + AT+CLCK="SC",2; install.sh stores
    that snapshot in the shared data directory for the containerized control plane.
    """
    if not reader_name.startswith("VoWiFi Modem SIM "):
        return None
    path = os.path.join(cfg.DATA_DIR, "modem-sim-bridge.json")
    try:
        with open(path, "r", encoding="utf-8") as f:
            meta = json.load(f)
    except Exception:
        return None
    cpin = str(meta.get("cpin") or "").upper()
    try:
        clck = int(meta.get("clck"))
    except (TypeError, ValueError):
        clck = None
    if "READY" in cpin and clck == 0:
        return False
    if "SIM PIN" in cpin or clck == 1:
        return True
    return None
    ...
    try:
        with _Tx(conn):
            ...
            try:
                ...
                pass
            modem_pin = _modem_pin_override(str(r))
            if modem_pin is not None:
                info.pin_enabled = modem_pin
                if modem_pin is False:
                    info.pin_tries = None
