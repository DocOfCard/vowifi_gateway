...
from . import store, engine, status as status_mod, sim, card, notify_push, lpa, estkme, usbreader, auth
    ...
    while True:
        try:
            ...
            current = {st["name"]: st for st in states
                       if not _is_internal_modem_reader(st["name"])}
            changed = False
...
_AUTH_OPEN = {"/api/auth/status", "/api/auth/setup", "/api/auth/login", "/api/engine/event"}

@app.middleware("http")
async def auth_middleware(request: Request, call_next):
    path = request.url.path
    if not path.startswith("/api/") or path in _AUTH_OPEN:
        return await call_next(request)
    sess = auth.session(request.cookies.get(auth.SESSION_COOKIE))
    if not sess:
        return JSONResponse({"detail": "authentication required"}, status_code=401)
    if request.method not in {"GET", "HEAD", "OPTIONS"}:
        supplied = request.headers.get("X-CSRF-Token", "")
        if not supplied or not hmac.compare_digest(supplied, sess.get("csrf", "")):
            return JSONResponse({"detail": "invalid CSRF token"}, status_code=403)
    request.state.auth = sess
    return await call_next(request)

@app.get("/api/auth/status")
def auth_status(request: Request):
    configured = auth.configured()
    sess = auth.session(request.cookies.get(auth.SESSION_COOKIE)) if configured else None
    return {"configured": configured, "authenticated": bool(sess),
            "username": auth.username() if configured else "admin",
            "csrf": sess.get("csrf") if sess else None}

@app.post("/api/auth/setup")
async def auth_setup(request: Request):
    if auth.configured(): raise HTTPException(409, "administrator already configured")
    body = await request.json()
    try: auth.setup(str(body.get("password") or ""), str(body.get("username") or "admin"))
    except ValueError as e: raise HTTPException(400, str(e))
    result = auth.login(str(body.get("username") or "admin"), str(body.get("password") or ""), request.client.host if request.client else "local")
    token, csrf = result
    r = JSONResponse({"ok": True, "csrf": csrf, "username": auth.username()})
    r.set_cookie(auth.SESSION_COOKIE, token, httponly=True, secure=True, samesite="strict", max_age=auth.SESSION_TTL, path="/")
    return r

@app.post("/api/auth/login")
async def auth_login(request: Request):
    peer = request.client.host if request.client else "local"
    delay = auth.throttled(peer)
    if delay: raise HTTPException(429, f"too many attempts; retry in {delay}s")
    body = await request.json(); result = auth.login(str(body.get("username") or ""), str(body.get("password") or ""), peer)
...
def auth_logout(request: Request):
    auth.logout(request.cookies.get(auth.SESSION_COOKIE)); r=JSONResponse({"ok":True}); r.delete_cookie(auth.SESSION_COOKIE,path="/"); return r

@app.post("/api/auth/change-password")
async def auth_change_password(request: Request):
    body=await request.json()
    try: auth.change_password(str(body.get("current_password") or ""), str(body.get("new_password") or ""))
    except ValueError as e: raise HTTPException(400,str(e))
    r=JSONResponse({"ok":True}); r.delete_cookie(auth.SESSION_COOKIE,path="/"); return r


# ----------------------------- SIM / readers -----------------------------

def _is_internal_modem_reader(name: str) -> bool:
    """Hide vsmartcard companion slots and the dedicated lpac transport from normal UI.

    They are transport artifacts, not additional physical SIM/eUICC chips. Keep their
    real PC/SC indices intact internally so lpac can address the hidden LPA reader.
    """
    n = str(name or "")
    if n.startswith("VoWiFi Modem eUICC LPA"):
        return True
    if n.startswith("VoWiFi Modem ") and n.endswith(" 00 01"):
        return True
    return False

def _public_reader_names() -> list[str]:
    return [n for n in sim.list_readers() if not _is_internal_modem_reader(n)]

@app.get("/api/readers")
def api_readers():
    return {"readers": _public_reader_names()}
...
async def api_cards():
    ...
    if not hub.scanned:
        ...
        def scan():
            ...
            for st in card.reader_states() or []:
                if _is_internal_modem_reader(st["name"]):
                    continue
    ...
    return {"cards": [c for c in hub.cards_list() if not _is_internal_modem_reader(c.get("name", ""))]}
