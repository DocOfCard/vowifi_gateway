...
def _env_for_reader(reader_name: str | None, aid: str | None = None) -> dict[str, str]:
    ...
    env["LPAC_APDU"] = "pcsc"
    env["LPAC_HTTP"] = "curl"
    # STANDALONE lpac keeps its private shared libraries under $VOWIFI_DATA/lpac/lib.
    # Driver .so files are dlopen()ed later and do not inherit the executable's RUNPATH,
    # so make the private lib dir explicit instead of requiring host-side patchelf.
    lpac_lib = os.path.join(cfg.DATA_DIR, "lpac", "lib")
    old_ld = env.get("LD_LIBRARY_PATH", "")
    env["LD_LIBRARY_PATH"] = lpac_lib + ((":" + old_ld) if old_ld else "")
    ...
    if reader_name:
        # Modem-backed SIM readers use bridge-owned logical channels for VoWiFi. eUICC
        # management is different: lpac/libeuicc must see the modem basic channel and be
        # allowed to manage ISD-R/logical channels itself. The bridge exposes a hidden
        # passthrough PC/SC reader for that purpose.
        transport_reader = reader_name
        if reader_name.startswith("VoWiFi Modem SIM "):
            transport_reader = env.get("VOWIFI_MODEM_LPA_READER",
                                       "VoWiFi Modem eUICC LPA 00 00")
        env["LPAC_APDU_PCSC_DRV_NAME"] = transport_reader
    else:
