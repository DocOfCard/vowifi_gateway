...
## Modem-resident SIM support (experimental)

This fork adds an opt-in bridge for SIMs that remain inside cellular modules such as
Quectel EC25-family and BAIWANG/QDC modules. The bridge uses ModemManager's guarded
AT command interface and `AT+CSIM`, allocates three UICC logical channels, and exposes
them through the virtualsmartcard VPCD PC/SC driver. The existing engine continues to
use PC/SC; the IKE/ESP/IMS code does not need a modem-specific path.

Enable it after the normal install (modem index from `mmcli -L`):
...
The bridge requires a module that supports `AT+CSIM` and ISO 7816 MANAGE CHANNEL.
It is intentionally opt-in because ModemManager must run with its command interface enabled.

## Administrator login

This fork also adds a single-administrator login. On first WebUI access you create the
administrator account. Passwords are scrypt-derived with a random salt; the credential
file is stored under the runtime data directory with mode 0600. Sessions use an
HttpOnly/Secure/SameSite=Strict cookie and state-changing API calls require a CSRF token.
