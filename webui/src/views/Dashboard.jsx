...
function UnprovisionedCard({ card, refresh, onProvision }) {
  ...
  // PIN state is authoritative when the backend knows it. A modem-backed eUICC can
  // legitimately have PIN facility disabled while an IMSI probe on a virtual logical
  // channel is unavailable, so 'missing IMSI' must not be rendered as 'SIM locked'.
  const locked = card.pin_enabled === true && !card.imsi
  ...
  return (
    <div onClick={(e) => e.stopPropagation()}>
      ...
      {!card.imsi && <div style={{ marginTop: 12, fontSize: 12.5, color: '#eab308' }}>
        PIN is not required, but IMSI is not readable on this reader yet. VoWiFi provisioning needs the active USIM identity.
      </div>}
      <button className="btn btn-primary" style={{ marginTop: 16 }} disabled={!card.imsi}
