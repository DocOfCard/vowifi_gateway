...
const emptyInstance = () => ({
  ...
  reader_index: 0, reader_port: '', reader_mode: 'single',
  reader_pin_index: 0, reader_swu_index: 0, reader_ims_index: 2,
  msisdn: '', smsc: '', enabled: true, apn: 'ims', idr_mode: 'apn', cp_mode: 'auto',
  use_reauth_id: true,
  proxy: { mode: 'direct', host: '', port: 1080, username: '', password: '', rdns: false },
  sip: {
...
export default function SimConfig({ instances, selected, refresh, cards, setSelected }) {
  ...
  useEffect(() => {
    ...
    setForm({ ...base, ...selected,
      proxy: { ...base.proxy, ...(selected.proxy || {}), password: '' },
      sip: { ...base.sip, ...(selected.sip || {}) } })
  }, [selected?.id])
  ...
  const save = async () => {
    ...
    try {
      ...
      delete body.pin
      if (pin) body.pin = pin
      if (body.proxy && !String(body.proxy.password || '').trim()) {
        body.proxy = { ...body.proxy }; delete body.proxy.password
      }
  ...
  return (
    <div style={{ maxWidth: 1000 }}>
      ...
      <div className="card" style={{ padding: 20 }}>
        ...
        <h4 style={{ marginBottom: 6 }}>SIM hardware backend</h4>
        <Field label="Reader mode">
          <select value={form.reader_mode || 'single'} onChange={(e) => upd({ reader_mode: e.target.value })}>
            <option value="single">Normal PC/SC / CCID reader</option>
            <option value="modem_bridge">Modem internal SIM (QDC507 / EC25 / ModemManager bridge)</option>
          </select>
        </Field>
        {form.reader_mode === 'modem_bridge' && <div style={{ marginTop: 8 }}>
          <div style={{ fontSize: 11, color: 'var(--text-mute)', marginBottom: 6 }}>
            Compatibility layout for Ubuntu vsmartcard: PIN keeper and SWu share reader 0; IMS uses reader 2. The unused companion slots may remain “Card removed”.
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
            <Field label="PIN reader"><input type="number" min="0" value={form.reader_pin_index ?? 0} onChange={(e) => upd({ reader_pin_index: +e.target.value, reader_index: +e.target.value, reader_port: '' })} /></Field>
            <Field label="SWu reader"><input type="number" min="0" value={form.reader_swu_index ?? 0} onChange={(e) => upd({ reader_swu_index: +e.target.value })} /></Field>
            <Field label="IMS reader"><input type="number" min="0" value={form.reader_ims_index ?? 2} onChange={(e) => upd({ reader_ims_index: +e.target.value })} /></Field>
          </div>
        </div>}

        <h4 style={{ marginBottom: 6, marginTop: 18 }}>SOCKS5 ePDG proxy</h4>
        <Field label="ePDG transport">
          <select value={form.proxy?.mode || 'direct'} onChange={(e) => upd({ proxy: { ...(form.proxy || {}), mode: e.target.value } })}>
            <option value="direct">Direct</option>
            <option value="socks5">SOCKS5 (UDP ASSOCIATE)</option>
          </select>
        </Field>
        {form.proxy?.mode === 'socks5' && <div>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 8 }}>
            <Field label="SOCKS5 server"><input value={form.proxy.host || ''} onChange={(e) => upd({ proxy: { ...form.proxy, host: e.target.value } })} placeholder="10.10.100.253" /></Field>
            <Field label="Port"><input type="number" min="1" max="65535" value={form.proxy.port || 1080} onChange={(e) => upd({ proxy: { ...form.proxy, port: +e.target.value } })} /></Field>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
            <Field label="Username"><input value={form.proxy.username || ''} onChange={(e) => upd({ proxy: { ...form.proxy, username: e.target.value } })} /></Field>
            <Field label="Password"><input type="password" value={form.proxy.password || ''} onChange={(e) => upd({ proxy: { ...form.proxy, password: e.target.value } })} placeholder="blank keeps saved password" /></Field>
          </div>
          <label style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
            <input type="checkbox" style={{ width: 'auto' }} checked={!!form.proxy.rdns} onChange={(e) => upd({ proxy: { ...form.proxy, rdns: e.target.checked } })} /> Resolve destination names through SOCKS5
          </label>
          <div style={{ fontSize: 11, color: 'var(--text-mute)', marginTop: 4 }}>IKE starts on UDP/500 through SOCKS5. Proxy mode then forces RFC3948 NAT-T; IKE, ESP and NAT keepalives use UDP/4500 through SOCKS5.</div>
        </div>}
