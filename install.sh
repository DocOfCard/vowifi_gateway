...
cmd_modem_bridge() {
  need_root
  modem="0"
  for a in $ARGS; do modem="$a"; break; done
  have systemctl || die "modem bridge requires systemd"
  # Re-running the installer is supported: stop the previous bridge first so its
  # SIGTERM cleanup releases any logical channels it owns before services restart.
  systemctl stop vowifi-modem-sim-bridge.service >/dev/null 2>&1 || true
  have mmcli || pkg_install modemmanager
  have ss || pkg_install iproute2

  # vsmartcard-vpcd is intentionally opt-in. Ubuntu's package uses a symlink for
  # libifdvpcd.so, so do NOT require -type f here.
  if have apt-get; then pkg_install vsmartcard-vpcd
  elif have dnf || have yum; then pkg_install vsmartcard-vpcd || die "install a vpcd PC/SC driver for your distro"
  else die "install vsmartcard-vpcd (virtualsmartcard PC/SC driver) first"
  fi
  vpcd_lib=$(find /usr/lib /usr/local/lib -name libifdvpcd.so -print -quit 2>/dev/null || true)
  [ -n "$vpcd_lib" ] || die "libifdvpcd.so not found after installing vsmartcard-vpcd"

  # Installing vsmartcard-vpcd on Ubuntu may pull the distro pcscd (1.9.x) back in.
  # Re-apply the project's PC/SC version lock here so host pcscd and the engine's
  # libpcsclite stay on the same ABI/protocol version.
  host_pcsc_ver=$(pcscd --version 2>/dev/null | sed -n '1s/.*version \([0-9][0-9.]*\).*/\1/p')
  if [ "$host_pcsc_ver" != "$PCSC_VERSION" ]; then
    info "vsmartcard changed host pcscd to ${host_pcsc_ver:-unknown}; restoring pinned $PCSC_VERSION"
    ensure_pcscd
    host_pcsc_ver=$(pcscd --version 2>/dev/null | sed -n '1s/.*version \([0-9][0-9.]*\).*/\1/p')
  fi
  [ "$host_pcsc_ver" = "$PCSC_VERSION" ] || die "could not pin host pcscd to $PCSC_VERSION"

  # Ubuntu/Debian vsmartcard 3.3 exposes TWO PC/SC slots per configured VPCD
  # instance. Only the first slot of each instance is used here. Two instances are
  # enough: pin_keeper shares the SWu reader after verifying CHV1, while IMS gets an
  # independent logical channel. This preserves the file-selection isolation that
  # matters (SWu vs IMS) without relying on unstable multi-instance slot numbering.
  mkdir -p /etc/vowifi_gateway/reader-conf-backup /etc/reader.conf.d
  if [ -e /etc/reader.conf.d/vpcd ] && [ ! -e /etc/vowifi_gateway/reader-conf-backup/vpcd ]; then
    mv /etc/reader.conf.d/vpcd /etc/vowifi_gateway/reader-conf-backup/vpcd
  fi
  cat >/etc/reader.conf.d/vowifi-modem-vpcd.conf <<EOF
FRIENDLYNAME "VoWiFi Modem SIM PIN-SWu"
DEVICENAME   /dev/null:0x8C7B
LIBPATH      $vpcd_lib
CHANNELID    0x8C7B

FRIENDLYNAME "VoWiFi Modem SIM IMS"
DEVICENAME   /dev/null:0x8C7D
LIBPATH      $vpcd_lib
CHANNELID    0x8C7D

# Internal management transport for lpac/eUICC. The control plane hides this
# reader from the normal dashboard and uses its first slot only.
FRIENDLYNAME "VoWiFi Modem eUICC LPA"
DEVICENAME   /dev/null:0x8C7F
LIBPATH      $vpcd_lib
CHANNELID    0x8C7F
EOF

  # pcscd's distro unit normally uses --auto-exit. That makes all VPCD TCP listeners
  # disappear after idle time while the modem bridge is still running. Keep pcscd
  # resident whenever the modem bridge is enabled.
  mkdir -p /etc/systemd/system/pcscd.service.d
  cat >/etc/systemd/system/pcscd.service.d/90-vowifi-modem-bridge.conf <<'EOF'
[Service]
ExecStart=
ExecStart=/usr/sbin/pcscd --foreground
EOF

  mm_bin=$(command -v ModemManager 2>/dev/null || true)
  [ -n "$mm_bin" ] || die "ModemManager executable not found"
  mkdir -p /etc/systemd/system/ModemManager.service.d
  cat >/etc/systemd/system/ModemManager.service.d/90-vowifi-command-interface.conf <<EOF
[Service]
ExecStart=
ExecStart=$mm_bin --debug
...
EOF
  ...
  # EC25/QDC507 can take 10-20 seconds to be recreated after ModemManager restarts.
  # Wait for the requested modem AND command passthrough instead of a fixed sleep.
  ready=0
  i=0
  while [ "$i" -lt 40 ]; do
    if mmcli -m "$modem" >/dev/null 2>&1 && mmcli -m "$modem" --command=AT >/dev/null 2>&1; then
      ready=1
      break
    fi
    i=$((i + 1))
  ...
  [ "$ready" = 1 ] || die "ModemManager modem $modem did not become AT-command ready within 40 seconds"

  # Record the modem-authoritative PIN facility state for the containerized control plane.
  # Empty VERIFY APDUs returning 63Cx are retry-counter probes, not reliable proof that
  # PIN1 is enabled on modem/eUICC paths. AT+CPIN? + AT+CLCK="SC",2 is authoritative.
  cpin_raw=$(mmcli -m "$modem" --command='AT+CPIN?' 2>/dev/null || true)
  clck_raw=$(mmcli -m "$modem" --command='AT+CLCK="SC",2' 2>/dev/null || true)
  cpin=$(printf '%s\n' "$cpin_raw" | sed -n 's/.*+CPIN:[[:space:]]*\([^'"'"']*\).*/\1/p' | head -n1)
  clck=$(printf '%s\n' "$clck_raw" | sed -n 's/.*+CLCK:[[:space:]]*\([01]\).*/\1/p' | head -n1)
  mkdir -p "$VOWIFI_DATA_DIR"
  printf '{"modem":"%s","cpin":"%s","clck":%s}\n' \
    "$modem" "${cpin:-unknown}" "${clck:-null}" >"$VOWIFI_DATA_DIR/modem-sim-bridge.json"

  systemctl restart pcscd.socket >/dev/null 2>&1 || true
  systemctl restart pcscd.service

  # Require the three first-slot VPCD listeners before starting the APDU bridge.
  ready=0
  i=0
  while [ "$i" -lt 10 ]; do
    if ss -lnt 2>/dev/null | grep -q ':35963 ' && ss -lnt 2>/dev/null | grep -q ':35965 ' && ss -lnt 2>/dev/null | grep -q ':35967 '; then
      ready=1
      break
    fi
    i=$((i + 1))
  ...
  [ "$ready" = 1 ] || die "pcscd did not open VPCD ports 35963/35965/35967"

  bridge="$REPO_DIR/host/modem_sim_bridge.py"
  [ -x "$bridge" ] || chmod +x "$bridge"
  mkdir -p /run/vowifi-modem-sim-bridge
  cat >/etc/systemd/system/vowifi-modem-sim-bridge.service <<EOF
[Unit]
Description=VoWiFi modem-resident SIM to VPCD bridge
After=ModemManager.service pcscd.service network.target
Requires=ModemManager.service pcscd.service

[Service]
Type=simple
ExecStart=/usr/bin/python3 $bridge --modem $modem --vpcd-host 127.0.0.1 --vpcd-ports 35963,35965,35967 --state-file /run/vowifi-modem-sim-bridge/state.json
Restart=on-failure
RestartSec=3
TimeoutStopSec=20
...
EOF
  systemctl daemon-reload
  systemctl enable vowifi-modem-sim-bridge.service >/dev/null
  systemctl restart vowifi-modem-sim-bridge.service
  sleep 2
  systemctl is-active --quiet vowifi-modem-sim-bridge.service || {
    journalctl -u vowifi-modem-sim-bridge.service -n 40 --no-pager >&2 || true
    die "modem SIM bridge failed to stay running"
  }

  info "modem SIM bridge enabled for ModemManager modem $modem"
  info "PC/SC mapping: PIN+SWu reader 0; IMS reader 2; hidden eUICC/LPA passthrough reader 4"
  info "verify with: pcsc_scan"
}

cmd_modem_bridge_disable() {
  need_root
  systemctl disable --now vowifi-modem-sim-bridge.service >/dev/null 2>&1 || true
  rm -f /etc/systemd/system/vowifi-modem-sim-bridge.service
  rm -f /etc/systemd/system/ModemManager.service.d/90-vowifi-command-interface.conf
  rm -f /etc/systemd/system/pcscd.service.d/90-vowifi-modem-bridge.conf
  rm -f /etc/reader.conf.d/vowifi-modem-vpcd.conf
  if [ -e /etc/vowifi_gateway/reader-conf-backup/vpcd ] && [ ! -e /etc/reader.conf.d/vpcd ]; then
    mv /etc/vowifi_gateway/reader-conf-backup/vpcd /etc/reader.conf.d/vpcd
  fi
  rm -rf /run/vowifi-modem-sim-bridge
  systemctl daemon-reload
  systemctl try-restart pcscd.socket >/dev/null 2>&1 || true
  systemctl try-restart pcscd.service >/dev/null 2>&1 || true
  systemctl try-restart ModemManager.service >/dev/null 2>&1 || true
  info "modem SIM bridge disabled"
}
...
${B}VoWiFi→SIP gateway installer${N}
  ...
  $0 modem-bridge [MODEM]    expose a modem-resident SIM (EC25/QDC/ModemManager) as virtual PC/SC readers
  $0 modem-bridge-disable    stop/remove the modem SIM bridge and ModemManager command drop-in
...
case "$CMD" in
  ...
  modem-bridge)       cmd_modem_bridge ;;
  modem-bridge-disable) cmd_modem_bridge_disable ;;
  patch)              cmd_patch ;;
  patch2)             cmd_patch2 ;;
